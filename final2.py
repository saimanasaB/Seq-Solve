...
def plot_distribution(jobs):
    ...
    axes[0].bar(deadline_counts.index, deadline_counts.values, color='skyblue')
    axes[0].set_title('Deadline Distribution')
    axes[0].set_xlabel('Deadline')
    axes[0].set_ylabel('Frequency')

    profit_counts = jobs['Profit'].value_counts().sort_index()
    axes[1].bar(profit_counts.index, profit_counts.values, color='lightgreen')
